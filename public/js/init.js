  ...
  "use strict";
	...
	var MatchAnor = {
		...
		init: function(){
			...
			MatchAnor.counter();
			MatchAnor.tippy();
			MatchAnor.copyToClipboard();
			MatchAnor.searchFilterOpener();
			MatchAnor.hashtag();
			MatchAnor.filterItems();
			MatchAnor.closeFilter();
			MatchAnor.hideText();
			MatchAnor.graph();
			MatchAnor.accordion();
			MatchAnor.product__activity_filter();
			MatchAnor.product__action();
			MatchAnor.triggerMenu();
			MatchAnor.megamenu();
			MatchAnor.contactForm();
			MatchAnor.selectFromPopup();
			MatchAnor.flickitySlider();
			MatchAnor.actionButton();
			MatchAnor.introTab();
		},
...
//				cw = (((width - (cols - 1) * gap) / cols) * 100) / width +'%';
		...
		flickitySlider: function(){
			if(!AnorIsSafari){
				MatchAnor.sliderWidth();
			}else{
				MatchAnor.safariFlickity();
			}
		...
		product__activity_filter: function(){
			...
			$('.anor_fn_product_activity .filter_list a').off().on('click',function(){
				...
				if(!e.hasClass('selected')){
					...
					MatchAnor.imgToSVG();
					MatchAnor.product_activity_filter_remove();
				}else{
							...
							external: function(context) {
								...
								if (tooltipModel.body) {
									...
									titleLines.forEach(function(title) {
										...
										title 		= MatchAnor.monthNames(month) + ' ' + day + ', ' + year;
